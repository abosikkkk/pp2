...
class Snake:
    ...
    def handle_keys(self, paused_state):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP and self.direction != DOWN:
                    ...
                    return not paused_state  # Toggle pause state
        return paused_state
...
def game_loop(username):
    ...
    while True:
        ...
        while snake.alive:
            screen.fill(BLACK)
            paused = snake.handle_keys(paused)
            if paused:
                draw_text(screen, "Paused. Press 'P' to continue.", SCREEN_WIDTH // 2 - 150, SCREEN_HEIGHT // 2, 36, WHITE)
                pygame.display.update()
                clock.tick(5)
                continue
